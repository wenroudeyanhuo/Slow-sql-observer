...
Slow SQL Observer 是一个基于 Go 的单 source MySQL 慢 SQL 采集与分析工具。它可以从慢日志文件或 `mysql.slow_log` 表读取慢查询事件，对重复 SQL 做 fingerprint 归一化，把原始记录和聚合结果写入独立分析库，并提供 HTTP API 与轻量 Web UI。

英文说明见 [README.md](README.md)。

## 项目定位

当前版本是一个“通用单 source 版本”，适合以下场景：

- 本地可直接读取的 MySQL 慢日志文件
- 可通过 SSH 拉取的远端慢日志文件
- 通过 `log_output=TABLE` 暴露 `mysql.slow_log` 的 MySQL 实例

当前版本不追求覆盖所有托管云数据库慢日志场景。尤其是下面这类情况，不在当前版本承诺范围内：
...
- 对 SQL 做规则归一化和 fingerprint 聚合
- 将原始记录和聚合指标写入独立分析库
- 提供总览、fingerprint 列表、详情、原始记录下钻页面
- 提供总览趋势图和单 fingerprint 趋势图
- 提供 source、collector、acquisition、discovery 等运行状态接口

## 架构图

完整图见 [docs/architecture.md](docs/architecture.md)。
...
- 如果 discovery 结果是 `mysql_table`，collector 会直接读 `mysql.slow_log`
- 如果 discovery 结果是 `mysql_file`，当前版本仍然需要文件访问能力，通常意味着还需要 SSH 相关配置
...
- 一个运行实例同时监听多个 source
- 托管 MySQL 在 `FILE` 模式下但没有 SSH 或云厂商日志 API 的场景
- `ssh_pull` 的密码登录
...
Slow SQL Observer 现在明确区分这两层含义：

- MySQL 的采集阈值
  通常来自 `slow_query_log`、`long_query_time` 等 MySQL 配置，它决定哪些 SQL 能进入慢日志源。
- Slow SQL Observer 的分析阈值
  由 `SSO_ANALYSIS_MIN_QUERY_TIME_SEC` 控制，它决定哪些已经采集到的记录会进入默认的总览、排行、详情和趋势视图。

例如：

- MySQL 可以用 `0.2s` 做采集，用于保留更多原始样本
- Slow SQL Observer 可以默认用 `1.0s` 做分析，避免首页被大量边缘慢 SQL 淹没
- 如果需要临时排查，API 和 UI 都可以通过 `minQueryTimeSec` 覆盖默认值
...
- 总览页趋势图
  用于观察最近一段时间整体慢 SQL 压力变化
- fingerprint 详情页趋势图
  用于观察某一条归一化 SQL 的历史波动
- 支持的时间桶
  `bucket=day` 与 `bucket=hour`
- 支持的时间窗口
  `days=1..30`，其中小时级趋势要求 `days <= 7`

趋势接口与页面遵循和排行榜一致的分析阈值规则，因此 `minQueryTimeSec` 会同时影响列表、详情和图表展示口径。

## 快速开始

### 方案 A：使用示例日志

如果只是做 smoke test，可以直接使用仓库里的示例日志：
...
4. 如果你希望页面显示 source 元数据，可以把 `SSO_SOURCE_DB_DSN` 指向同一个本地 MySQL。
5. 如果希望默认只分析更明显的慢 SQL，可以设置 `SSO_ANALYSIS_MIN_QUERY_TIME_SEC=1`。
...
然后打开和 `SSO_SERVER_ADDR` 一致的地址。

例如：

- 如果 `SSO_SERVER_ADDR=:8080`，访问 [http://localhost:8080](http://localhost:8080)
- 如果 `SSO_SERVER_ADDR=:8191`，访问 [http://localhost:8191](http://localhost:8191)
...
5. 到下面这些接口或页面确认是否已经采到数据：
   ...
   - 可以额外比较 `/api/slow-sql/fingerprints?minQueryTimeSec=0.2` 和 `/api/slow-sql/fingerprints?minQueryTimeSec=1`

这条本地慢日志采集链路已经用真实 MySQL 文件完成过验证。

## 这次版本收口

当前版本可以视为一个可落地的单 source 版本，适合：

- 从本地可读慢日志文件采集
- 从 `mysql.slow_log` 表采集
- 监听一个 MySQL 实例下的多个业务库

当前版本没有解决“所有云数据库慢日志接入问题”。最主要的未覆盖场景是：托管 MySQL 只输出 `FILE`，但你既拿不到 SSH，也拿不到云厂商日志 API。
...
- 当前只支持单 source
- 暂不支持托管 MySQL 的 FILE-only 慢日志自动获取
...
当前代码已经包含 source-aware、acquisition、analysis threshold 和 trend analytics 等主要实现结果，但并不代表之前讨论过的每条探索方向都已经进入正式产品承诺范围。
