...
当前版本可以视为一个“通用单 source 版本”，适合以下场景：

- 本地可直接读取的 MySQL 慢日志文件
- 可通过 SSH 拉取的远端慢日志文件
- 通过 `log_output=TABLE` 暴露 `mysql.slow_log` 的 MySQL 实例

这个版本不追求覆盖所有托管云数据库慢日志场景。尤其是下面这类情况，不在当前版本承诺范围内：
...
- Slow SQL Observer 的分析阈值
  由 `SSO_ANALYSIS_MIN_QUERY_TIME_SEC` 控制，它决定哪些已经采集到的记录会进入默认的总览、排行、详情视图。
...
当前代码已经包含 source-aware 和 acquisition 层的主要实现结果，但并不代表之前讨论过的每条探索方向都已经进入正式产品承诺范围。
